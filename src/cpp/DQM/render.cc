...
private:
  ...
  // ----------------------------------------------------------------------
  // Render a pull-value comaprison of object and reference
  void
  doRenderPullValComparison(TCanvas &c,
                          VisDQMImgInfo &i,
                          VisDQMObject *objs,
                          size_t numobjs,
                          const VisDQMRenderInfo &ri,
                          std::vector<TObject *> &nukem)
  {
    ...
    // copy all objects, such that we can safely modify pulls[0].
    std::vector<VisDQMObject> pulls(objs, objs+numobjs);
    ...
    if (ref_profile != nullptr) {
      pull_hist = ref_profile->ProjectionXY("pull_hist");
    } else {
      pull_hist = dynamic_cast<TH2*> (ref_hist->Clone("pull_hist"));
    }
    nukem.push_back(pull_hist);
    ...
    for (auto x = 1; x <= ref_hist->GetNbinsX(); x++) {
      for (auto y = 1; y <= ref_hist->GetNbinsY(); y++){
        ...
        // Check if max_pull
        max_pull = std::max(max_pull, std::abs(new_pull));
    ...
    pull_hist->SetTitle(pull_hist->GetTitle());
    gStyle->SetPalette(kLightTemperature);
    gStyle->SetNumberContours(255);
    pull_hist->SetOption("colz");

    pulls[0].object = pull_hist;
    pulls[0].reference = nullptr;
    doRenderOrdinary(c, i, &pulls[0], numobjs, ri, nukem);
  }

  double
  pull(double bin1, double binerr1, double bin2, double binerr2) {
    return (bin1 - bin2) / (sqrt(binerr1*binerr1 + binerr2*binerr2));
  }

  std::tuple<double, double>
  get_poisson_errors(double bin1, double bin2) {
    auto alpha = 1 - 0.6827;
